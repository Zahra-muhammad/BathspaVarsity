...
function displayProductsByCategory() {
  ...
  if (containers.length < 4) {
    console.warn('⚠️ Not enough containers found. Expected at least 4, found:', containers.length);
    return;
  }
  ...
  // 🎯 PLAYER-ONLY: Display Jerseys - Container index 3
  if (containers[3]) {
    if (jerseys.length > 0) {
      const html = jerseys.map(product => createProductCard(product)).join('');
      containers[3].insertAdjacentHTML('beforeend', html);
      console.log('✅ Added', jerseys.length, 'Jerseys to container[3] (PLAYER ONLY)');
    } else {
      // Show message if no jerseys in database
      containers[3].innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.5);">
          <p style="font-size: 1.2rem; margin-bottom: 10px;">No jerseys available yet</p>
          <p style="font-size: 0.9rem;">Check back soon for exclusive player jerseys!</p>
        </div>
      `;
      console.log('ℹ️ No jerseys found in database');
    }
  }
